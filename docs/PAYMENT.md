...
Nexora has a built-in payment system that enables user self-service top-up without deploying a separate payment service.
...
- [Migrating from an External Payment Bridge](#migrating-from-an-external-payment-bridge)
...
>
> - **Domestic channel / CNY settlement** - [ZPay](https://z-pay.cn/) (`https://z-pay.cn/`): supports Alipay and WeChat Pay aggregation for domestic checkout scenarios.
> - **International channel / USD settlement** - [Stripe](https://stripe.com/) (`https://stripe.com/`): supports global cards and multiple local payment methods for international billing.
>
...
## Migrating from an External Payment Bridge

If you previously used a separate payment bridge or custom checkout service, you can migrate to the built-in payment system:

### Key Differences

| Aspect | External Payment Bridge | Built-in Payment |
|--------|-----------|-----------------|
| Deployment | Separate service | Built into Nexora, no extra deployment |
| Payment Methods | EasyPay, Alipay, WeChat, Stripe | Same |
| Configuration | Environment variables + separate admin UI | Unified in Nexora admin dashboard |
| Top-up Integration | Via Admin API callback | Internal processing, more reliable |
| Subscription Plans | Supported | Not yet (planned) |
| Order Management | Separate admin interface | Integrated in Nexora admin dashboard |

### Migration Steps

1. Enable payment in the Nexora admin dashboard and configure providers (use the same payment credentials)
2. Update webhook callback URLs to Nexora's callback endpoints
3. Verify that new orders are processed correctly via built-in payment
4. Decommission the old payment bridge service

> **Note**: Historical order data from the old payment bridge is not migrated automatically. Keep the legacy service available temporarily if you still need historical queries.
