          ...
          <div class="grid gap-3">
            ...
            >
              <div class="flex items-center gap-4">
                <!-- 澶嶉€夋锛堢鐢ㄧ姸鎬侊級 -->
                <div class="flex-shrink-0">
                  <div class="flex h-5 w-5 items-center justify-center rounded-md border-2 border-primary-400 bg-primary-500 dark:border-primary-600 dark:bg-primary-600">
...
const load = async () => {
  ...
  try {
    ...
    groupConfigs.value = groups.value.map((g) => ({
      ...
      // 涓撳睘鍒嗙粍锛氭鏌ユ槸鍚﹀湪 allowed_groups 涓?      // 鍏紑鍒嗙粍锛氬缁堥€変腑
...
const handleSave = async () => {
  ...
  try {
    ...
    // 鏋勫缓 group_rates
    // - 鏈夋柊涓撳睘鍊嶇巼: 璁剧疆涓鸿鍊?    // - 鍘熸湰鏈変笓灞炲€嶇巼浣嗙幇鍦ㄨ娓呯┖: 璁剧疆涓?null锛堣〃绀哄垹闄わ級
    ...
    for (const c of groupConfigs.value) {
      ...
      if (c.customRate !== null) {
        ...
        // 鍘熸湰鏈変笓灞炲€嶇巼锛岀幇鍦ㄨ娓呯┖锛岄渶瑕佹樉寮忓垹闄?        groupRates[c.groupId] = null
      }
    }
