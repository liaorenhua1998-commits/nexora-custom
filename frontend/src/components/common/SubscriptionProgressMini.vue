<template>
  <div v-if="hasActiveSubscriptions" class="relative" ref="containerRef">
    <button
      @click="toggleTooltip"
      class="flex cursor-pointer items-center gap-2 rounded-xl border border-white/60 bg-white/60 px-3 py-1.5 shadow-[0_14px_34px_rgba(15,23,42,0.06),inset_0_1px_0_rgba(255,255,255,0.52)] backdrop-blur-2xl transition-all hover:-translate-y-0.5 hover:border-primary-200/70 hover:bg-white/70 dark:border-white/10 dark:bg-white/5 dark:hover:border-primary-400/20 dark:hover:bg-white/10"
    ...
    <transition name="dropdown">
      <div
        v-if="tooltipOpen"
        class="dropdown right-0 mt-2 w-[340px] overflow-hidden rounded-2xl"
      >
        <div class="border-b border-white/45 p-3 dark:border-white/10">
        ...
        <div class="max-h-64 overflow-y-auto">
          <div
            ...
            :key="subscription.id"
            class="border-b border-white/35 p-3 last:border-b-0 dark:border-white/5"
          >
            ...
            <div class="space-y-1.5">
              <div
                ...
                class="flex items-center gap-2 rounded-lg bg-gradient-to-r from-primary-50 to-accent-50 px-2.5 py-1.5 dark:from-primary-500/15 dark:to-accent-500/15"
              >
        ...
        <div class="border-t border-white/45 p-2 dark:border-white/10">
          <router-link
            to="/subscriptions"
            @click="closeTooltip"
...
function getMaxUsagePercentage(sub: UserSubscription): number {
  ...
  if (sub.group?.daily_limit_usd) percentages.push(((sub.daily_usage_usd || 0) / sub.group.daily_limit_usd) * 100)
  if (sub.group?.weekly_limit_usd) percentages.push(((sub.weekly_usage_usd || 0) / sub.group.weekly_limit_usd) * 100)
  if (sub.group?.monthly_limit_usd) percentages.push(((sub.monthly_usage_usd || 0) / sub.group.monthly_limit_usd) * 100)
...
function getProgressDotClass(sub: UserSubscription): string {
  if (isUnlimited(sub)) return 'bg-primary-500'
...
function handleClickOutside(event: MouseEvent) {
  if (containerRef.value && !containerRef.value.contains(event.target as Node)) closeTooltip()
}
