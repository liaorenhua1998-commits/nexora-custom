<template>
  <div class="space-y-4">
    <AuthOAuthButton
      :disabled="disabled"
      :label="t('auth.oidc.signIn', { providerName: normalizedProviderName })"
      @click="startLogin"
    >
      <template #icon>
        <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary-100 text-[11px] font-bold text-primary-700">
          {{ providerInitial }}
        </span>
      </template>
    </AuthOAuthButton>
