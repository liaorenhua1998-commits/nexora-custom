...
function sanitizePublicSettings(data: PublicSettings): PublicSettings {
  const siteSubtitle = typeof data.site_subtitle === 'string' ? data.site_subtitle.trim() : ''
  return {
    ...data,
    site_subtitle: /\?{2,}/.test(siteSubtitle) ? '' : siteSubtitle,
  }
}
...
export async function getPublicSettings(): Promise<PublicSettings> {
  ...
  return sanitizePublicSettings(data)
}
