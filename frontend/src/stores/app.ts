...
function sanitizePublicSettings(config: PublicSettings): PublicSettings {
  const siteSubtitle = normalizePublicText(config.site_subtitle)
  return {
    ...config,
    site_subtitle: isCorruptedSubtitle(siteSubtitle) ? '' : siteSubtitle,
  }
}
...
function isCorruptedSubtitle(value: string): boolean {
  return /\?{2,}/.test(value)
}
...
export const useAppStore = defineStore('app', () => {
  ...
  const siteName = ref<string>('Nexora')
  ...
  function applySettings(config: PublicSettings): void {
    const sanitized = sanitizePublicSettings(config)
    if (typeof window !== 'undefined') {
      window.__APP_CONFIG__ = { ...sanitized }
    }
    cachedPublicSettings.value = sanitized
    siteName.value = sanitized.site_name || 'Nexora'
    siteLogo.value = sanitized.site_logo || '/logo.png'
    siteVersion.value = sanitized.version || ''
    contactInfo.value = sanitized.contact_info || ''
    apiBaseUrl.value = sanitized.api_base_url || ''
    docUrl.value = sanitized.doc_url || ''
