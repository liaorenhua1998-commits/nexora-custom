                          ...
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              ...
                              <span
                                ...
                                :class="row.configured ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'"
                              >
          ...
          <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-800">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {{ inputDetailRow.endpoint || '-' }} 路 {{ inputDetailRow.provider || '-' }} / {{ inputDetailRow.model || '-' }}
                </p>
              </div>
...
const configForm = reactive({
  ...
  queue_size: 32768,
  block_status: 403,
  block_message: 'Request blocked by moderation rules. Please revise the input and try again.',
  email_on_hit: true,
...
function applyConfig(config: ContentModerationConfig) {
  ...
  configForm.block_message = config.block_message || 'Request blocked by moderation rules. Please revise the input and try again.'
...
async function saveConfig() {
  ...
  try {
    const payload: UpdateContentModerationConfig = {
      ...
      block_message: configForm.block_message || 'Request blocked by moderation rules. Please revise the input and try again.',
...
function resultBadgeClass(row: ContentModerationLog): string {
  ...
  if (row.flagged) return 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300'
