  ...
  <AppLayout>
          ...
          <template #cell-account_count="{ row }">
            <div class="space-y-0.5 text-xs">
              <div>
                ...
                <span
                  class="ml-1 font-medium text-primary-600 dark:text-primary-400"
                  >{{
    ...
    >
      ...
      >
        ...
        <!-- 鏀寔鐨勬ā鍨嬬郴鍒楋紙浠?antigravity 骞冲彴锛?-->
        ...
        <!-- MCP XML 鍗忚娉ㄥ叆锛堜粎 antigravity 骞冲彴锛?-->
        <div v-if="createForm.platform === 'antigravity'" class="border-t pt-4">
          <div class="mb-1.5 flex items-center gap-1">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
        ...
        <!-- Claude Code 瀹㈡埛绔檺鍒讹紙浠?anthropic 骞冲彴锛?-->
        <div v-if="createForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <!-- 闄嶇骇鍒嗙粍閫夋嫨锛堜粎褰撳惎鐢?claude_code_only 鏃舵樉绀猴級 -->
        ...
        <!-- OpenAI Messages 璋冨害閰嶇疆锛堜粎 openai 骞冲彴锛?-->
        <div
        ...
        <!-- 璐﹀彿杩囨护鎺у埗 (OpenAI/Antigravity/Anthropic/Gemini) -->
        <div
          v-if="
        ...
        >
          ...
          <div class="flex items-center justify-between">
            <div>
              <label class="text-sm text-gray-600 dark:text-gray-400">Require privacy-configured accounts only</label>
              
              
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{
                  createForm.require_privacy_set
                    ? "Enabled. Accounts without privacy settings are excluded."
                     : "Disabled."
                }}
              </p>
            </div>
        ...
        <!-- 鏃犳晥璇锋眰鍏滃簳锛堜粎 anthropic/antigravity 骞冲彴锛屼笖闈炶闃呭垎缁勶級 -->
        <div
          v-if="
...
// Fallback group options for create.
...
// Invalid-request fallback group options for create.
...
// 澶嶅埗璐﹀彿鐨勬簮鍒嗙粍閫夐」锛堝垱寤烘椂锛? 浠呭寘鍚浉鍚屽钩鍙颁笖鏈夎处鍙风殑鍒嗙粍
...
// Copy-account source group options for edit.
...
// Lightweight account shape used by routing rules.
...
// Search accounts for the current routing rule.
...
// Handle account search input focus.
const onAccountSearchFocus = (
  ...
  // Trigger a search when the dropdown opens without cached results.
  if (!accountSearchResults.value[key]?.length) {
    searchAccounts(key);
  }
};

// Add create-form routing rule.
const addCreateRoutingRule = () => {
  createModelRoutingRules.value.push({ pattern: "", accounts: [] });
};

// Remove create-form routing rule.
...
// 灏?API 鏍煎紡鐨勮矾鐢辫鍒欒浆鎹负 UI 鏍煎紡锛堥渶瑕佸姞杞借处鍙峰悕绉帮級
...
const editForm = reactive({
  ...
  // Copy accounts from other groups.
  copy_accounts_from_group_ids: [] as number[],
  // 浠庡垎缁勫鍒惰处鍙?  copy_accounts_from_group_ids: [] as number[],
  // 鍒嗙粍绾?RPM 闄愬埗锛堟瘡鐢ㄦ埛姣忓垎閽熸渶澶ц姹傛暟锛? = 涓嶉檺鍒讹級
  rpm_limit: 0 as number,
});
...
const handleCreateGroup = async () => {
  ...
  try {
    // Build request payload including routing config.
    const requestData = {
      ...createForm,
    ...
    // Convert cleared number inputs into null for unlimited semantics.
...
const handleEdit = async (group: AdminGroup) => {
  ...
  editForm.copy_accounts_from_group_ids = [];
  // Reset copied-account selections whenever edit opens.
  editForm.rpm_limit = group.rpm_limit ?? 0;
  editForm.mcp_xml_inject = group.mcp_xml_inject ?? true;
  editForm.copy_accounts_from_group_ids = []; // 澶嶅埗璐﹀彿瀛楁姣忔缂栬緫鏃堕噸缃负绌?  editForm.rpm_limit = group.rpm_limit ?? 0;
...
const handleUpdateGroup = async () => {
  ...
  submitting.value = true;
  try {
    // 杞崲 fallback_group_id: null -> 0 (鍚庣浣跨敤 0 琛ㄧず娓呴櫎)
    const payload = {
      ...editForm,
      daily_limit_usd: normalizeOptionalLimit(
...
// 鐩戝惉 subscription_type 鍙樺寲锛岃闃呮ā寮忔椂 is_exclusive 榛樿涓?true
watch(
...
// Close the account search dropdown when clicking outside.
const handleClickOutside = (event: MouseEvent) => {
  const target = event.target as HTMLElement;
  // 妫€鏌ユ槸鍚︾偣鍑诲湪涓嬫媺妗嗘垨杈撳叆妗嗗唴
