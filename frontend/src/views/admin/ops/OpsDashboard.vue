<template>
  <component :is="rootComponent" v-bind="rootProps">
    <div :class="[isFullscreen ? 'space-y-6 pb-10' : 'space-y-6 pb-12']">
      <div
        v-if="errorMessage"
        class="rounded-2xl bg-red-50/90 p-4 text-sm text-red-600 shadow-[0_16px_40px_rgba(239,68,68,0.08)] dark:bg-red-900/20 dark:text-red-400"
      >
        {{ errorMessage }}
      </div>
...
const rootComponent = computed(() => (isFullscreen.value ? StandalonePageShell : AppLayout))
const rootProps = computed(() => (isFullscreen.value ? { maxWidth: 'full' as const } : {}))
