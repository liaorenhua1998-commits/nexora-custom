<template>
  <AppLayout>
    <div class="mx-auto flex max-w-lg flex-col items-center space-y-6 py-8">
      <div class="glass-card w-full p-6 text-center">
        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-3xl bg-primary-100 text-primary-600 dark:bg-primary-900/30 dark:text-primary-400">
          <svg v-if="qrUrl" class="h-8 w-8" viewBox="0 0 24 24" fill="currentColor">
            <path d="M3 3h8v8H3V3zm2 2v4h4V5H5zm8-2h8v8h-8V3zm2 2v4h4V5h-4zM3 13h8v8H3v-8zm2 2v4h4v-4H5zm10-2h2v2h-2v-2zm-2 2h2v2h-2v-2zm4 0h2v2h-2v-2zm2 2h2v2h-2v-2zm-6 2h2v2h-2v-2zm2-2h2v2h-2v-2zm2 2h2v2h-2v-2z" />
          </svg>
          <Icon v-else name="externalLink" size="xl" />
        </div>
        <h2 class="mt-4 text-xl font-semibold text-gray-900 dark:text-white">
          {{ qrUrl ? scanTitle : t('payment.qr.payInNewWindow') }}
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
          {{ qrUrl ? t('payment.qr.waitingPayment') : t('payment.qr.payInNewWindowHint') }}
        </p>
      </div>

      <div v-if="qrUrl" class="glass-card w-full p-6">
        <canvas ref="qrCanvas" class="mx-auto"></canvas>
      </div>

      <div v-if="qrUrl && !expired && scanHint" class="surface-subtle w-full px-5 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
        {{ scanHint }}
      </div>

      <div v-if="expired" class="glass-card w-full p-6 text-center">
        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-3xl bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400">
          <Icon name="clock" size="xl" />
        </div>
        <p class="mt-4 text-lg font-medium text-red-500">{{ t('payment.qr.expired') }}</p>
        <button class="btn btn-primary mt-5" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
      </div>

      <div v-else class="surface-subtle w-full p-5 text-center">
