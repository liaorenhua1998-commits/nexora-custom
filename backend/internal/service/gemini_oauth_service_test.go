...
func TestGeminiOAuthService_GenerateAuthURL_RedirectURIStrategy(t *testing.T) {
	...
	// The Gemini CLI OAuth credentials are not embedded in this repository.
	// Tests set dummy values via env to simulate operator-provided configuration.
	t.Setenv(geminicli.GeminiCLIOAuthClientIDEnv, testGeminiCLIClientID)
	...
	tests := []struct {
		...
		},
		{
			...
			cfg: &config.Config{
				Gemini: config.GeminiConfig{
					OAuth: config.GeminiOAuthConfig{
						ClientID:     testGeminiCLIClientID,
