...
const (
	...
	defaultLoginAgreementDate    = "2026-05-11"
)
	...
	return []LoginAgreementDocument{
		{
			...
			ContentMD: "# Terms of Service\n\nWelcome to Nexora. Please review billing, moderation, upstream model availability, and payment rules before using the platform.",
		},
		{
			ID:        "privacy-policy",
			Title:     "Privacy Policy",
			ContentMD: "# Privacy Policy\n\nWe only collect the data required for authentication, gateway relay, payment recharge, risk control, and auditing.",
		},
		{
			ID:        "acceptable-use",
			Title:     "Acceptable Use Policy",
			ContentMD: "# Acceptable Use Policy\n\nDo not use the platform for illegal content, abusive traffic, quota evasion, or attempts to bypass platform risk controls.",
		},
		{
			ID:        "refund-policy",
			Title:     "Recharge and Refund Policy",
			ContentMD: "# Recharge and Refund Policy\n\nSuccessful payments are credited to platform balance by default. Refunds follow platform notices, ticket agreements, and payment provider rules.",
		},
	}
}
...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		SiteName:                         s.getStringOrDefault(settings, SettingKeySiteName, "Nexora"),
		SiteLogo:                         s.getStringOrDefault(settings, SettingKeySiteLogo, "/logo.png"),
		SiteSubtitle:                     sanitizePublicSiteSubtitle(s.getStringOrDefault(settings, SettingKeySiteSubtitle, "企业级 AI 网关与中转平台")),
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result := &SystemSettings{
		...
		SiteName:                         s.getStringOrDefault(settings, SettingKeySiteName, "Nexora"),
		SiteLogo:                         s.getStringOrDefault(settings, SettingKeySiteLogo, "/logo.png"),
		SiteSubtitle:                     s.getStringOrDefault(settings, SettingKeySiteSubtitle, "企业级 AI 网关与中转平台"),
		APIBaseURL:                       settings[SettingKeyAPIBaseURL],
		ContactInfo:                      settings[SettingKeyContactInfo],
		DocURL:                           settings[SettingKeyDocURL],
		HomeContent:                      settings[SettingKeyHomeContent],
...
func sanitizePublicSiteSubtitle(value string) string {
...
// EnsureOperationalDefaults backfills key runtime settings without overriding existing values.
func (s *SettingService) EnsureOperationalDefaults(ctx context.Context) error {
	if err := s.InitializeDefaultSettings(ctx); err != nil {
		return err
	}

	existing, err := s.settingRepo.GetAll(ctx)
	if err != nil {
		return fmt.Errorf("load settings for backfill: %w", err)
	}
	...
	defaults := map[string]string{
		...
		SettingKeySiteName:                             "Nexora",
		SettingKeySiteLogo:                             "/logo.png",
		SettingKeySiteSubtitle:                         "企业级 AI 网关与中转平台",
		SettingKeyAPIBaseURL:                           "",
		SettingKeyContactInfo:                          "",
		SettingKeyDocURL:                               "",
		SettingKeyHomeContent:                          "",
		...
		SettingMaxRechargeAmount:                       "10000",
		SettingDailyRechargeLimit:                      "50000",
		...
		SettingProductNamePrefix:                       "Nexora Recharge",
		SettingProductNameSuffix:                       "Balance",
		SettingHelpImageURL:                            "/share-cover.png",
		SettingHelpText:                                "Use your documentation center or payment guide to publish supported methods, expected arrival time, and refund rules.",
		SettingCancelRateLimitOn:                       "false",
		SettingCancelRateLimitMax:                      "10",
		SettingCancelWindowSize:                        "1",
		SettingCancelWindowUnit:                        "day",
		SettingCancelWindowMode:                        "rolling",
		SettingKeyTableDefaultPageSize:                 "20",
		SettingKeyTablePageSizeOptions:                 "[10,20,50,100]",
		SettingKeyCustomMenuItems:                      "[]",
		SettingKeyCustomEndpoints:                      "[]",
