...
func flattenOpenAIChatCompletionsContent(raw any) string {
	switch v := raw.(type) {
	case string:
...
func extractOpenAIChatCompletionsText(body []byte) string {
	var payload struct {
		Choices []struct {
			Message struct {
	...
	for _, choice := range payload.Choices {
		if text := flattenOpenAIChatCompletionsContent(choice.Message.Content); text != "" {
			return text
		}
...
func (s *AccountTestService) testOpenAIChatCompletionsAPIKeyConnection(
	...
	if resp.StatusCode != http.StatusOK {
		...
		if upstreamMsg := extractOpenAIErrorMessage(body); upstreamMsg != "" {
			return s.sendErrorAndEnd(c, fmt.Sprintf("API returned %d: %s", resp.StatusCode, upstreamMsg))
		}
		return s.sendErrorAndEnd(c, fmt.Sprintf("API returned %d: %s", resp.StatusCode, string(body)))
	}

	text := extractOpenAIChatCompletionsText(body)
	if text == "" {
		text = "(empty response)"
	}
	s.sendEvent(c, TestEvent{Type: "content", Text: text})
