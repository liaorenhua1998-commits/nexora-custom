...
func (s *OpenAIGatewayService) resolveOpenAIChannelRequestedModel(ctx context.Context, groupID *int64, requestedModel string) string {
	requestedModel = strings.TrimSpace(requestedModel)
	if requestedModel == "" || groupID == nil || s.channelService == nil {
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64) (*Account, error) {
	...
	routedModel := s.resolveOpenAIChannelRequestedModel(ctx, groupID, requestedModel)

	// 1. 尝试粘性会话命中
	// Try sticky session hit
	if account := s.tryStickySessionHit(ctx, groupID, sessionHash, routedModel, excludedIDs, requireCompact, stickyAccountID); account != nil {
		return account, nil
	}
	...
	selected, compactBlocked := s.selectBestAccount(ctx, groupID, accounts, routedModel, excludedIDs, requireCompact)
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool) (*AccountSelectionResult, error) {
	...
	if sessionHash != "" {
		...
		if accountID > 0 && !isExcluded(accountID) {
			account, err := s.getSchedulableAccount(ctx, accountID)
			if err == nil {
				clearSticky := shouldClearStickySession(account, routedModel)
				if clearSticky {
					_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
				}
				if !clearSticky && isOpenAIAccountEligibleForRequest(account, routedModel, false) {
					account = s.recheckSelectedOpenAIAccountFromDB(ctx, account, routedModel, requireCompact)
					if account == nil {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
					} else if needsUpstreamCheck && s.isUpstreamModelRestrictedByChannel(ctx, *groupID, account, routedModel, requireCompact) {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
					} else {
	...
	for i := range accounts {
		...
		if routedModel != "" && !acc.IsModelSupported(routedModel) {
			continue
		}
		if needsUpstreamCheck && s.isUpstreamModelRestrictedByChannel(ctx, *groupID, acc, routedModel, requireCompact) {
			continue
		}
		baseCandidateCount++
	...
	if err != nil {
		...
		for _, acc := range ordered {
			fresh := s.resolveFreshSchedulableOpenAIAccount(ctx, acc, routedModel, false)
			if fresh == nil {
				continue
			}
			fresh = s.recheckSelectedOpenAIAccountFromDB(ctx, fresh, routedModel, requireCompact)
			if fresh == nil {
				continue
			}
			if needsUpstreamCheck && s.isUpstreamModelRestrictedByChannel(ctx, *groupID, fresh, routedModel, requireCompact) {
				continue
			}
		...
		if len(available) > 0 {
			...
			for _, item := range selectionOrder {
				fresh := s.resolveFreshSchedulableOpenAIAccount(ctx, item.account, routedModel, false)
				if fresh == nil {
					continue
				}
				fresh = s.recheckSelectedOpenAIAccountFromDB(ctx, fresh, routedModel, requireCompact)
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	channelRequestedModel := s.resolveOpenAIChannelRequestedModel(ctx, apiKeyGroupID(apiKey), reqModel)
	passthroughEnabled := account.IsOpenAIPassthroughEnabled()
	if passthroughEnabled {
		// 透传分支只需要轻量提取字段，避免热路径全量 Unmarshal。
		reasoningEffort := extractOpenAIReasoningEffortFromBody(body, reqModel)
		passthroughBody := originalBody
		if channelRequestedModel != "" && channelRequestedModel != reqModel {
			passthroughBody = ReplaceModelInBody(passthroughBody, channelRequestedModel)
		}
		return s.forwardOpenAIPassthrough(ctx, c, account, passthroughBody, reqModel, channelRequestedModel, reasoningEffort, reqStream, startTime)
	}
	...
	if channelRequestedModel != "" && channelRequestedModel != reqModel {
		logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Channel model mapping applied: %s -> %s (account: %s, isCodexCLI: %v)", reqModel, channelRequestedModel, account.Name, isCodexCLI)
		reqBody["model"] = channelRequestedModel
		bodyModified = true
		markPatchSet("model", channelRequestedModel)
	}

	// 对所有请求执行模型映射（包含 Codex CLI）。
	billingModel := account.GetMappedModel(channelRequestedModel)
	if billingModel != channelRequestedModel {
		logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Account model mapping applied: %s -> %s (account: %s, isCodexCLI: %v)", channelRequestedModel, billingModel, account.Name, isCodexCLI)
...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	currentBodyModel := strings.TrimSpace(gjson.GetBytes(body, "model").String())
	passthroughSourceModel := strings.TrimSpace(channelMappedModel)
	if passthroughSourceModel == "" {
		passthroughSourceModel = reqModel
	}
	upstreamPassthroughModel := resolveOpenAIForwardModel(account, passthroughSourceModel, "")
	if upstreamPassthroughModel != "" && upstreamPassthroughModel != currentBodyModel {
	...
	if isOpenAIResponsesCompactPath(c) {
		compactSourceModel := passthroughSourceModel
		if upstreamPassthroughModel != "" {
			compactSourceModel = upstreamPassthroughModel
		}
		compactMappedModel := resolveOpenAICompactForwardModel(account, compactSourceModel)
		if compactMappedModel != "" && compactMappedModel != compactSourceModel {
