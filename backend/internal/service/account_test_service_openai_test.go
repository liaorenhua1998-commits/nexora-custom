...
func TestAccountTestService_OpenAI429PersistsSnapshotAndRateLimitState(t *testing.T) {
	...
	resp := newJSONResponse(http.StatusTooManyRequests, `{"error":{"type":"usage_limit_reached","message":"limit reached","resets_at":1777283883},"subscription":{"plan_type":"plus"}}`)
...
func TestAccountTestService_OpenAIAPIKeyCompatFallsBackToChatCompletions(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx, recorder := newTestContext()

	resp := newJSONResponse(http.StatusOK, `{"choices":[{"message":{"content":"compat-ok"}}]}`)
	upstream := &queuedHTTPUpstream{responses: []*http.Response{resp}}
...
func TestAccountTestService_OpenAIAPIKeyCompat401SetsPermanentError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx, recorder := newTestContext()

	resp := newJSONResponse(http.StatusUnauthorized, `{"error":{"message":"bad compat token"}}`)
	repo := &openAIAccountTestRepo{}
	upstream := &queuedHTTPUpstream{responses: []*http.Response{resp}}
	svc := &AccountTestService{
		accountRepo:  repo,
	...
	require.Equal(t, account.ID, repo.setErrorID)
	require.Contains(t, repo.setErrorMsg, "Authentication failed (401)")
	require.Contains(t, recorder.Body.String(), "bad compat token")
}
