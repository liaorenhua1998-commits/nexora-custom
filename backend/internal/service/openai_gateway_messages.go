...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	channelRequestedModel := s.resolveOpenAIChannelRequestedModel(ctx, apiKeyGroupID(getAPIKeyFromContext(c)), normalizedModel)
	clientStream := anthropicReq.Stream // client's original stream preference

	// 2. Model mapping
	billingModel := resolveOpenAIForwardModel(account, channelRequestedModel, defaultMappedModel)
	...
	logFields := []zap.Field{
		...
		zap.String("channel_model", channelRequestedModel),
