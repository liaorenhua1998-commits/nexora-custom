...
func validateAutoSetupAdminConfig(cfg *SetupConfig) error {
	...
	if cfg.Admin.Password == "" {
		if strings.EqualFold(strings.TrimSpace(cfg.Server.Mode), "release") {
			return fmt.Errorf("ADMIN_PASSWORD is required when AUTO_SETUP=true and SERVER_MODE=release")
		}
		return nil
	}
...
func AutoSetupFromEnv() error {
	...
	cfg := &SetupConfig{
		...
		Admin: AdminConfig{
			Email:    getEnvOrDefault("ADMIN_EMAIL", "admin@nexora.local"),
	...
	if err := validateAutoSetupAdminConfig(cfg); err != nil {
		return err
	}
