...
const testGeminiCLIClientID = "test-gemini-cli-client-id"

func setTestGeminiCLIEnv(t *testing.T) {
	t.Helper()
	t.Setenv(GeminiCLIOAuthClientIDEnv, testGeminiCLIClientID)
...
func TestBuildAuthorizationURL(t *testing.T) {
	...
	checks := []string{
		"response_type=code",
		"client_id=" + testGeminiCLIClientID,
		"redirect_uri=",
		"state=test-state",
...
func TestBuildAuthorizationURL_RequiresConfiguredClient(t *testing.T) {
	...
	if err == nil {
		t.Fatal("BuildAuthorizationURL() 未配置 Gemini CLI OAuth client 时应报错")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientIDEnv) {
		t.Errorf("错误消息应包含 %s，实际: %v", GeminiCLIOAuthClientIDEnv, err)
	}
}
...
func TestEffectiveOAuthConfig_GoogleOne(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantClientID: testGeminiCLIClientID,
...
func TestEffectiveOAuthConfig_NoEnvClient(t *testing.T) {
	t.Setenv(GeminiCLIOAuthClientIDEnv, "")
	t.Setenv(GeminiCLIOAuthClientSecretEnv, "")

	_, err := EffectiveOAuthConfig(OAuthConfig{}, "code_assist")
	if err == nil {
		t.Fatal("不设置环境变量时应返回错误")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientIDEnv) {
		t.Errorf("错误消息应包含 %s，实际: %v", GeminiCLIOAuthClientIDEnv, err)
	}
