...
//
// If ClientID/ClientSecret is not provided, this falls back to the configured Gemini CLI OAuth client.
//
...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	// Fall back to the operator-configured Gemini CLI OAuth client when not configured.
	// SECURITY: This repo does not embed OAuth credentials; both values must be provided via env.
	if effective.ClientID == "" && effective.ClientSecret == "" {
		...
		return OAuthConfig{}, infraerrors.New(http.StatusBadRequest, "GEMINI_OAUTH_CLIENT_NOT_CONFIGURED", "OAuth client not configured: please set both client_id and client_secret (or leave both empty to use the configured Gemini CLI client)")
	}

	isBuiltinClient := isConfiguredGeminiCLIClient(effective.ClientID)
...
func isConfiguredGeminiCLIClient(clientID string) bool {
	expected := strings.TrimSpace(os.Getenv(GeminiCLIOAuthClientIDEnv))
	return expected != "" && strings.TrimSpace(clientID) == expected
}
