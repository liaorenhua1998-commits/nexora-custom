...
//   - channelId / channelIdAlipay / channelIdWxpay
//   - channel_id / channel_id_alipay / channel_id_wxpay
//   - cid / cidAlipay / cidWxpay (legacy aliases)
...
func (e *EasyPay) createRedirectPayment(req payment.CreatePaymentRequest) (*payment.CreatePaymentResponse, error) {
	...
	if channelID := e.resolveChannelID(req.PaymentType); channelID != "" {
...
func (e *EasyPay) createAPIPayment(ctx context.Context, req payment.CreatePaymentRequest) (*payment.CreatePaymentResponse, error) {
	...
	var resp struct {
		...
		PayURL2   string `json:"payurl2"` // Non-standard H5 mobile payment URL
		QRCode    string `json:"qrcode"`
		URLScheme string `json:"urlscheme"` // WeChat mini-program / app redirect URL
	}
...
func (e *EasyPay) QueryOrder(ctx context.Context, tradeNo string) (*payment.QueryOrderResponse, error) {
	...
	if resp.Code != easypayCodeSuccess {
		return nil, fmt.Errorf("easypay query error: %s", resp.Msg)
	}
...
func (e *EasyPay) firstConfigValue(keys ...string) string {
	if e == nil {
		return ""
	}
	for _, key := range keys {
		for actualKey, value := range e.config {
