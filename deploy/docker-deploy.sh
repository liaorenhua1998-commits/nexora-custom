#!/bin/bash
# =============================================================================
# Nexora Docker Deployment Preparation Script
# =============================================================================
# This script prepares deployment files for Nexora:
#   - Copies docker-compose.local.yml and .env.example from the local repo
...
main() {
    echo ""
    echo "=========================================="
    echo "  Nexora Deployment Preparation"
    echo "=========================================="
    echo ""
    ...
    if [ ! -f "docker-compose.local.yml" ] || [ ! -f ".env.example" ]; then
        print_error "docker-compose.local.yml or .env.example is missing in the current directory."
        print_error "Run this script from the deploy directory of the Nexora source tree."
        exit 1
    fi

    print_info "Preparing docker-compose.yml from local branded sources..."
    ...
    echo ""
    echo "  3. View logs:"
    echo "     docker-compose logs -f nexora"
    echo ""
    echo "  4. Access Web UI:"
    echo "     http://localhost:8080"
    echo ""
    print_info "ADMIN_PASSWORD has been generated into .env for release AUTO_SETUP."
    print_info "Store it securely and rotate it after first login if your policy requires."
    echo ""
}
