# Nexora Deployment Files

This directory contains files for deploying the Nexora branded gateway on Linux servers.
...
| `docker-compose.prod.yml` | Production Docker Compose profile for public internet deployment |
| `docker-deploy.sh` | Local/general Docker deployment preparation script |
| `docker-deploy-prod.sh` | Production deployment preparation script |
| `.env.example` | Docker environment variables template |
| `.env.production.example` | Production environment template |
| `backup-postgres.sh` | PostgreSQL backup helper for the production compose profile |
| `restore-postgres.sh` | PostgreSQL restore helper for the production compose profile |
| `monitor-stack.sh` | Host-side readiness / container health monitor |
| `finalize-production-bootstrap.sh` | Disables first-boot bootstrap after admin password rotation |
| `install-prod-systemd.sh` | Installs backup / monitor systemd timers for production Docker deploys |
| `nexora-backup.service` | One-shot backup systemd unit |
| `nexora-backup.timer` | Daily backup systemd timer |
| `nexora-monitor.service` | One-shot production monitor systemd unit |
| `nexora-monitor.timer` | Repeating production monitor systemd timer |
| `nginx.nexora.conf.example` | HTTPS reverse proxy example |
| `PRODUCTION_CHECKLIST.md` | Pre-launch hardening checklist |
| `HONGKONG_SERVER_DEPLOY.md` | Step-by-step Hong Kong VPS deployment guide |
...
If this instance will serve real users on the public internet, use these files:
...
- `install-prod-systemd.sh`
- `nginx.nexora.conf.example`
- `PRODUCTION_CHECKLIST.md`
- `HONGKONG_SERVER_DEPLOY.md`

Quick path:

```bash
cd deploy
chmod +x docker-deploy-prod.sh backup-postgres.sh monitor-stack.sh restore-postgres.sh finalize-production-bootstrap.sh install-prod-systemd.sh
./docker-deploy-prod.sh
docker compose --env-file ./runtime/.env.production -f docker-compose.prod.yml up -d --build
```

This production profile binds Nexora to `127.0.0.1` by default, stores generated secrets in `./runtime/secrets/`, bootstraps invite-only / risk-control defaults into the database, and is intended to sit behind Nginx or Caddy on `80/443`.
...
Use the automated preparation script for the easiest local or single-host setup:

```bash
# Run from the Nexora source tree
cd deploy
...
- Uses the local branded `docker-compose.local.yml` and `.env.example`
...
- Displays generated credentials (POSTGRES_PASSWORD, ADMIN_PASSWORD, JWT_SECRET, etc.)
...
# View logs
docker compose -f docker-compose.local.yml logs -f nexora
...
```bash
# Enter the deploy directory from this source tree
cd deploy
...
**Recommendation:** Use `docker-compose.local.yml` for local/self-hosted single-node deployments, and `docker-compose.prod.yml` for public internet production.

### Local Relay Verification

This repository now includes a local OpenAI-compatible mock upstream for end-to-end relay testing.
...
What it does:
- creates the `nexora-openai` group
- creates or updates the `mock-openai-upstream` upstream account
- creates or updates the `mock-openai-channel` channel

The mock upstream is internal-only and is used to verify:
- API key auth
- group routing
- upstream forwarding
- `/v1/models`
...
1. On first run, the system automatically:
   ...
   - Creates admin account (release deployments must provide `ADMIN_PASSWORD`)
   - Writes config.yaml

2. No manual Setup Wizard needed - just configure `.env` and start

3. For release deployments, set `ADMIN_PASSWORD` in `.env` before startup.
   The provided `docker-deploy.sh` helper now generates and writes one into `.env`
   so the application does not need to print bootstrap credentials to logs.
...
# Restart Nexora only
docker compose -f docker-compose.local.yml restart nexora
...
| `ADMIN_EMAIL` | No | `admin@nexora.local` | Admin email |
| `ADMIN_PASSWORD` | **Yes (release AUTO_SETUP)** | - | Admin password |
...
Nexora supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://nexora.example/releases)
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 nexora
...
# View detailed logs
docker compose logs --tail=100 nexora
...
Nexora supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> Tip: collect TLS fingerprint samples from your own trusted environment or internal capture service before creating production profiles.
